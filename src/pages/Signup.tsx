...
export default function Signup() {
  const { user, isError, error, isLoading } = useAppSelector(
    (state) => state.user
  );
  const {
    register,
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="form-control w-full">
                ...
                {errors.email?.type === "required" && (
                  <p className="text-start" role="alert">
                    email is required
                  </p>
                )}
              </div>
              <div className="form-control w-full">
                ...
                <label className="label mt-1">
                  <span className="label-text">
                    already signup? <Link to="/login">login Now</Link>
                  </span>
                </label>
              </div>
              ...
              <input
                type="submit"
                value="Signup Now"
                className="btn btn-primary bg-secondary1  border-0 mx-auto mt-4 w-full"
              />
            </form>
          </div>
        </div>
